...
export async function before(m, {conn, isAdmin, isBotAdmin, usedPrefix}) {
  ...
  const fakemek = {'key': {'participant': '0@s.whatsapp.net', 'remoteJid': '0@s.whatsapp.net'}, 'message': {'groupInviteMessage': {'groupJid': '51995386439-1616969743@g.us', 'inviteCode': 'm', 'groupName': 'P', 'caption': '??? ?????? - ???', 'jpegThumbnail': null}}};
  if (chat.antiTraba && m.text.length > 5000) { // Cantidad m¨¢xima de caracteres aceptados en un mensaje//
    ...
    } else if (!bot.restrict) return m.reply('[ ! ] Para realizar acciones de eliminacion, mi creador tiene que activar el modo restringido!');
  }
  return !0;
}